...
export default function OnboardingPage() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-950 text-slate-100 selection:bg-purple-500/30">
      {/* Glows */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 -z-10 h-[400px] w-[600px] rounded-full bg-purple-600/10 blur-[130px] pointer-events-none" />
      <div className="absolute bottom-0 right-10 -z-10 h-[300px] w-[300px] rounded-full bg-blue-600/5 blur-[120px] pointer-events-none" />
      ...
      <main className="flex-1 max-w-3xl w-full mx-auto px-6 py-8 flex flex-col justify-between">
        ...
        <div className="flex-1 space-y-6 mb-6">
          {messages.map((msg) => (
            ...
            >
              {/* Bot Avatar */}
              {msg.sender === "bot" && (
                <div className="h-9 w-9 rounded-xl bg-slate-900 border border-slate-800 flex items-center justify-center text-purple-400 shrink-0 shadow-inner">
              ...
              <div className="flex flex-col gap-2 max-w-[85%]">
                <div
                  className={`rounded-2xl px-4 py-3 text-sm leading-relaxed border ${
                    msg.sender === "user"
                      ? "bg-purple-600/10 border-purple-500/20 text-purple-100 rounded-tr-none self-end"
                      : "bg-slate-900/40 border-slate-900 text-slate-200 rounded-tl-none backdrop-blur-md"
                  }`}
                >
                ...
                {msg.sender === "bot" && msg.options && (
                  <div className="mt-2 flex flex-wrap gap-2 animate-in fade-in duration-300 delay-100">
                      ...
                      return (
                        <button
                          ...
                          className={`text-xs px-3.5 py-2 rounded-xl border font-medium transition-all ${
                            isSelected
                              ? "bg-purple-600 border-purple-500 text-white shadow-lg shadow-purple-600/20 scale-[1.03]"
                              : "bg-slate-900/60 border-slate-800 text-slate-300 hover:border-slate-700 hover:bg-slate-800 hover:text-white"
                          }`}
                        >
                    ...
                    {msg.multipleSelect && (
                      <Button
                        ...
                        className="bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 text-white rounded-xl shadow-md border-0"
                      >
                        Kirim
              ...
              {/* User Avatar */}
              {msg.sender === "user" && (
                <div className="h-9 w-9 rounded-xl bg-purple-600/20 border border-purple-500/30 flex items-center justify-center text-purple-300 shrink-0 shadow-inner">
          ...
          {isTyping && (
            <div className="flex items-start gap-4">
              <div className="h-9 w-9 rounded-xl bg-slate-900 border border-slate-800 flex items-center justify-center text-purple-400 shrink-0">
                <Bot className="h-5 w-5 animate-pulse" />
              </div>
              <div className="bg-slate-900/40 border border-slate-900/80 rounded-2xl rounded-tl-none px-4 py-3 backdrop-blur-md flex items-center gap-1">
          ...
          {step > 5 && !isTyping && (
            <div className="flex justify-center pt-8 animate-in zoom-in-95 duration-500">
              <Card className="bg-slate-900/40 border-purple-500/20 max-w-md w-full p-6 text-center backdrop-blur-md relative overflow-hidden shadow-2xl">
                <div className="absolute top-0 right-0 h-[100px] w-[100px] rounded-full bg-purple-600/10 blur-[40px] -z-10" />
                <div className="h-12 w-12 rounded-2xl bg-purple-600/20 border border-purple-500/30 mx-auto flex items-center justify-center text-purple-400 mb-4">
                  <Sparkles className="h-6 w-6" />
                </div>
                <h3 className="text-lg font-bold text-white mb-2">Company Memory Siap Dibangun</h3>
                ...
                <Button
                  ...
                  className="w-full bg-gradient-to-r from-purple-600 via-pink-600 to-blue-600 hover:from-purple-500 hover:via-pink-500 hover:to-blue-500 text-white font-semibold shadow-lg shadow-purple-600/30 rounded-xl transition-all duration-300 hover:scale-[1.02]"
                >
        ...
        {step === 1 && !isTyping && (
          <div className="border-t border-slate-900 pt-4 bg-slate-950/80 backdrop-blur-md sticky bottom-0">
            <div className="relative flex items-center bg-slate-900 border border-slate-800 rounded-xl focus-within:border-purple-500/50 focus-within:ring-2 focus-within:ring-purple-600/25 transition-all p-1.5">
              <input
                type="text"
                ...
                className="flex-1 bg-transparent px-3 py-2 text-sm border-0 focus:outline-none focus:ring-0 text-white"
              />
              <Button
                size="sm"
                onClick={() => handleSend()}
                disabled={!inputText.trim()}
                className="bg-purple-600 hover:bg-purple-500 text-white shrink-0 rounded-lg shadow"
              >
                Kirim
